...
class MJCFBenchmarks:
    """Benchmark class for testing mujoco-usd-converter against multiple MJCF repositories."""

    def __init__(
        self,
        mjcf_repository_path_list: list[str],
        report_output_dir: str = "benchmarks",
        conversion_output_dir: str = "benchmarks",
    ):
        for mjcf_repository_path in mjcf_repository_path_list:
        ...
        self.mjcf_benchmarks: list[MJCFBenchmark] = [
            MJCFBenchmark(mjcf_repository_path, report_output_dir, conversion_output_dir) for mjcf_repository_path in mjcf_repository_path_list
        ]
    ...
    def run_benchmarks(self) -> list[BenchmarkResult]:
        results = []
        for benchmark in self.mjcf_benchmarks:
                ...
                writer.writerow(
                    {
                        ...
                        "Variant URL": result.dataset_url if asset_display else "",
...
"""
        ...
        # Sort results by asset name, then variant name
        sorted_results = sorted(self.results, key=lambda r: (r.repository_name, r.asset_name, r.variant_name))
        ...
        for result in sorted_results:
            ...
            if is_new_asset:
                ...
                asset_link = f'<a href="{result.dataset_url}" target="_blank" style="{link_style}">{result.asset_name}</a>'
...
"""
        ...
        table_header = (
            "| Repository | Asset | Variant | Success | [Verified (Manual)](#manual-annotation-instructions) | "
        ...
        for result in sorted_results:
            ...
            # Only show asset name and link for first variant in each group
            asset_display = f"**[{result.asset_name}]({result.dataset_url})**" if is_new_asset else ""
            previous_asset = result.asset_name
...
class MJCFBenchmark:
    """Main benchmark class for testing mujoco-usd-converter against mujoco_menagerie, etc."""

    def __init__(
        self,
        mjcf_repository_path: str,
        report_output_dir: str = "benchmarks",
        conversion_output_dir: str = "benchmarks",
        ...
        if self.mjcf_repository_url:
            default_branch = get_default_branch(self.mjcf_repository_url)
            self.mjcf_repository_base_url = f"{self.mjcf_repository_url}/tree/{default_branch}/"
        else:
            self.mjcf_repository_base_url = None
        ...
        self.report_output_dir = Path(report_output_dir)
        self.conversion_output_dir = Path(conversion_output_dir)
        self.use_temp_mjcf_directory = False
    ...
    def _setup_mjcf_files_from_repository(self) -> Path:
        ...
        if self.local_mjcf_directory and self.local_mjcf_directory.exists():
            logger.info("[%s] Using existing MJCF repository at: %s", self.repository_name, self.local_mjcf_directory)
        ...
        logger.info("[%s] Cloning MJCF repository to: %s", self.repository_name, mjcf_files_path)
        try:
            subprocess.run(
            ...
            logger.error("[%s] Failed to clone %s: %s", self.repository_name, self.mjcf_repository_url, e)
    ...
    def _find_mjcf_models(self) -> list[tuple[str, str, Path]]:
        ...
        for asset_name, annotation in self.annotations.items():
            ...
            asset_dir = self.local_mjcf_directory / self.base_dir / asset_name
    ...
    def _convert_model(self, asset_name: str, model_name: str, mjcf_file_path: Path) -> BenchmarkResult:
        """Convert a single MJCF file and return the benchmark result."""
        logger.info("[%s] Converting MJCF file: %s", self.repository_name, mjcf_file_path)
        ...
        result = BenchmarkResult(
            ...
            dataset_url=(
                urljoin(self.mjcf_repository_base_url, f"{mjcf_file_path.relative_to(self.local_mjcf_directory)}")
                if self.mjcf_repository_base_url
                else mjcf_file_path
            ),
            local_path=mjcf_file_path,
            success=False,
            error_count=0,
            warning_count=0,
            error_message="",
        ...
        model_output_dir = self.conversion_output_dir / self.repository_name / asset_name / model_name
    ...
    def run_benchmark(self) -> list[BenchmarkResult]:
        ...
        # Setup MJCF repository
        self._setup_mjcf_files_from_repository()

        # Find all models
        models = self._find_mjcf_models()
        ...
        for i, (asset_name, model_name, mjcf_path) in enumerate(models, 1):
            ...
            result = self._convert_model(asset_name, model_name, mjcf_path)
    ...
    def cleanup(self):
        ...
        if self.temp_dir_context:
            try:
                self.temp_dir_context.cleanup()
                logger.info("[%s] Cleaned up temporary MJCF directory", self.repository_name)
            except Exception as e:
                logger.warning("[%s] Failed to clean up temporary MJCF directory: %s", self.repository_name, str(e))
            finally:
                self.temp_dir_context = None
