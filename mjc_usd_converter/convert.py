...
class Converter:
    ...
    def convert(self, input_file: pathlib.Path, output_dir: pathlib.Path) -> Sdf.AssetPath:
        """
        ...
        Tf.Status(f"Converting {input_file} into {output_dir}")
        spec = mujoco.MjSpec.from_file(str(input_file.absolute()))
        ...
        if self.params.flatten:
            ...
            asset_dir = output_dir.absolute().as_posix()
            asset_format = "usda"
